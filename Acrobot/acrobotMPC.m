...
A2 = A_fun(eqb2(1), eqb2(2), eqb2(3), eqb2(4), 0);
B2 = B_fun(eqb2(1), eqb2(2), eqb2(3), eqb2(4), 0);
C2 = [1 0 0 0;
    0 1 0 0];
%Euler disc
A2 = eye(4) + dt*A2;
...
ncol = nu;
%Building Z
Z2 = zeros(Np*nrow, Np*ncol);
for i = 1:Np
    for j = 1:i
        subblock = Ca2 * Aa2^(i-j) * Ba2;
        vrow = (i-1) * nrow + [1:nrow];
        vcol = (j-1) * ncol + [1:ncol];
        Z2(vrow, vcol) = subblock;
    end
end

Ksave2 = (R + Z2' * Q * Z2) \ Z2' * Q;
...
for i = 2:steps
    ...
    elseif swflag == 3
        x_a = [x - x_prev;
            C2*x];
        rp = repmat(eqb2(1:2), Np, 1); %just duplicate and extend ref Np times
        delta_u = Ksave2 * (rp - W2*x_a);
